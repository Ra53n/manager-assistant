...
enum InvariantValidator {
    ...
    /// Блок `[INVARIANTS]` для промпта: перечень + КОНСТРУКТИВНОЕ требование (дай ответ
    /// на разрешённых альтернативах, запрещённые НЕ упоминай вообще) + жёсткий отказ
    /// только при реальной невозможности.
    static func promptBlock(_ invs: [Invariant]) -> String {
        ...
        return """
        [INVARIANTS — ОБЯЗАТЕЛЬНЫЕ ОГРАНИЧЕНИЯ, соблюдай строго]
        \(list)
        Действуй так:
        - Дай ПОЛНОЦЕННЫЙ ответ ПО СУЩЕСТВУ запроса, используя ТОЛЬКО разрешённые \
        альтернативы. Запрещённый инструмент/тему замени допустимыми аналогами. Почти \
        любой запрос выполним без запрещённого — приложи усилие найти замену.
        - ПОЛНОСТЬЮ избегай запрещённых слов: НЕ пиши их ВООБЩЕ — ни в плане, ни в коде, \
        ни в пояснениях, ни в скобках, ни в сравнениях («в отличие от …»), ни даже чтобы \
        сказать, что ты их избегаешь. Любое вхождение запрещённого слова = нарушение; \
        если напрашивается сравнение с запрещённым — просто опусти его.
        - Если текущий шаг НЕ затрагивает эти ограничения — выполняй обычно, не упоминай \
        ограничения и не выводи маркеров.
        - ТОЛЬКО если запрос ФИЗИЧЕСКИ невозможно выполнить без прямого нарушения (а не \
        «так удобнее») — ПЕРВОЙ строкой выведи «\(violationMarker): <точное название из \
        списка>», кратко объясни конфликт и предложи допустимую альтернативу. Это крайний \
        случай: НЕ выводи маркер, если ответ можно дать на разрешённых альтернативах.
        """
    }
}
