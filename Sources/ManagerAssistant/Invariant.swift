...
enum InvariantValidator {
    ...
    static func promptBlock(_ invs: [Invariant]) -> String {
        ...
        [INVARIANTS]
        \(list)
        Это ОБЯЗАТЕЛЬНЫЕ ограничения проекта. Соблюдай их во всём, что предлагаешь и пишешь.
        - Если текущая задача/шаг НЕ затрагивают эти ограничения — просто выполняй обычно, \
        НЕ упоминай инварианты и НЕ выводи никаких маркеров.
        - НЕ используй запрещённое и НЕ пиши запрещённые термины в ответе (включая код, \
        пояснения и комментарии) — любое их вхождение считается нарушением.
        - ТОЛЬКО если выполнить запрос пользователя НЕВОЗМОЖНО без прямого нарушения \
        инварианта — не делай нарушающую работу и не строй план: ПЕРВОЙ строкой выведи \
        «\(violationMarker): <точное название из списка>», далее кратко объясни конфликт и \
        предложи допустимую альтернативу. Этот маркер выводи ТОЛЬКО при реальной \
        невозможности; НИКОГДА не выводи его как заметку, гипотезу или «ложное срабатывание».
        """
    }
}
