...
- **Конечный автомат задачи (FSM) — формальная детерминированная модель** — задача
  ...
  вывод в ленту через `addMessage(..., state:step:total:)`. **Связь прогонов одного чата**
  (новый TaskContext затирает старый — без этого «какая была цель диалога?» отвечалась по
  случайным RAG-фрагментам): (1) `PipelinePrompts.dialogContext` — компактный блок
  `[КОНТЕКСТ ДИАЛОГА]` в каждый `buildPrompt` (снапшот на старте прогона: реплики
  пользователя + итоговые ответы `state==nil/.answer`, БЕЗ этапного шума, последние 6 по
  500 симв.; текущая задача не дублируется); (2) `TaskContext.carryGuidance` — уточнения/
  ограничения пользователя переносятся в guidance нового прогона (кап 6 последних). **Краш-устойчивость**:
...
- **Локальный RAG (база знаний) — pipeline индексации + ретрив в чате** — ОТДЕЛЬНЫЙ от
  ...
  • **Второй этап ретрива (реранкинг/фильтрация + query rewrite)** — полный пайплайн в
    ...
    **Фрагменты = ДАННЫЕ**: заголовок `buildBlock` явно объявляет фрагменты справочными
    данными из заметок (в базе лежат и заметки-промпты со своими «целями/ограничениями» —
    без оговорки модель выдавала их за установки текущего диалога и следовала им).
